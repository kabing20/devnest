...
<div class="form-group">
    <label>View More Link(https://w3codegenerator.com) :</label>
    <input type="url" name="view_more" placeholder="Enter Title" value="{{ old('view_more') ?? $row->view_more ?? '' }}"
           class="form-control{{ ($errors->has('title'))?' form-error':'' }}">
    @if($errors->has('view_more'))
        <div class="form-error-message">{{ $errors->first('view_more') }}</div>
    @endif
</div>
