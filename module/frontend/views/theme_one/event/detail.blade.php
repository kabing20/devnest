@extends($theme_path.'.layouts.master_layout')

@section('content')

    <section class="event-detail">
        <div class="breadcrumb-section">
            <div class="container">
                <div class="img-div"><img src="{{ asset('static_asset/frontend/theme_one/images/event-detail/bread1.png') }}" alt=""></div>
                <div class="breadcrumb-info wow fadeInUp animated" data-wow-offset="0" data-wow-delay="300ms">
                    <span>Home  >  Events & Promotions  > {{ $data['row']->title }}</span>
                    <h4>{{ $data['row']->title }} {{ isset($data['row']->inaguration)?" Inauguration by ".$data['row']->inaguration:"" }}</h4>
                    <div>
                        <span class="date">{{ $data['row']->date }}</span>
                        <span class="address">{{ $data['row']->location }}</span>
                    </div>

                </div>
            </div>
        </div>
        <div class="description-section">
            <div class="container">
                <div class="row">
                    <div class="col-md-8 wow fadeInLeft animated" data-wow-offset="0" data-wow-delay="300ms">
                        <img src="{{ $data['row']->image('700_700') }}" alt="">
                        {{-- {!! \Illuminate\Support\Str::limit($data['row']->description,150) !!} --}}
                        <p>{!! $data['row']->description ?? '' !!}</p>
                    </div>
                    <div class="col-md-4 wow fadeInRight animated" data-wow-offset="0" data-wow-delay="300ms">
                        <div class="title"><h4>Latest Events</h4></div>

                        @if(isset($data['latest']) && count($data['latest'])>0)
                            @foreach($data['latest'] as $row)
                                @continue($row->id == $data['row']->id)
                                <div class="latest">
                                    <div class="row">
                                        <div class="col-md-3">
                                            <a href="{{ route('frontend.event.detail',$row->slug) }}"><img src="{{ $row->image('50_50') }}" alt=""></a>
                                        </div>
                                        <div class="col-md-9">
                                            <h5><a href="{{ route('frontend.event.detail',$row->slug) }}">{{ $row->title }}</a></h5>
                                            <p>{!! \Illuminate\Support\Str::limit($row->description,150) !!}</p>
                                        </div>
                                    </div>
                                </div>
                            @endforeach
                        @endif
                    </div>
                </div>
            </div>
        </div>


        <div class="similar-event">
            <div class="container">
                <div class="event-title wow fadeInUp animated" data-wow-offset="0" data-wow-delay="300ms">
                    <h4>SIMILAR EVENTS</h4>
                </div>
                <div class="event-item">
                    <div class="row">
                        @if(isset($data['latest']) && count($data['latest'])>0)
                            @foreach($data['latest'] as $row)
                                @continue($row->id == $data['row']->id)
                                <div class="col-md-4 wow fadeInUp animated" data-wow-offset="0" data-wow-delay="300ms">
                                    <div class="image"><a href="{{ route('frontend.blog.detail',$row->slug) }}"><img src="{{ $row->image('700_700') }}" alt=""></a></div>
                                    <div class="dates">
                                        <span>{{ $row->date }}</span>
                                        <span>{{ $row->location }}</span>
                                    </div>
                                    <div class="name"><a href="{{ route('frontend.event.detail',$row->slug) }}"><h5>{{ $row->title }}</h5></a></div>
                                    <div class="info">
                                        <span>{{ \Illuminate\Support\Str::limit($row->description,150) }}</span>
                                    </div>
                                </div>
                            @endforeach
                        @endif
                    </div>
                </div>
            </div>
        </div>

    </section>

    @endsection
