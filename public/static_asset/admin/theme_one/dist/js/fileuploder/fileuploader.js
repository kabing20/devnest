$(document).ready(function() {
	...
	var $fileuploader = $('input.gallery_media').fileuploader({
			...
			onSave: function(dataURL, item) {
				...
				if (item.appended && item.data.listProps) {
					...
					item.html.find('.fileuploader-action-popup').hide();
 	                route=window.location.origin+'/image/productcrop';
					$.get(route, {name: item.name,product_id:item.data.product_id, id: item.data.listProps.id, _editor: JSON.stringify(item.editor)}, function() {
		...
		afterRender: function(listEl, parentEl, newInputEl, inputEl) {
			var api = $.fileuploader.getInstance(inputEl),
				$plusInput = listEl.find('.fileuploader-input');
				$plusInput.addClass('file-main-1')

				// console.log(.addClass('file-main-1'))
		...
		onRemove: function(item) {
			...
			if (item.data.listProps)
				...
				route=window.location.origin+'/image/productremove';


				$.get(route,{name:item.name, product_id:item.data.product_id});
		},
